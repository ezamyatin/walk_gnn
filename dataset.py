...
class EgoDataset(IterableDataset):
    def __init__(self, ego_net_path):
        self.ego_net_path = ego_net_path

    @staticmethod
    def read_ego_net(ego_net_path, limit=None):
        cur_ego_id = -1
        cur_ego_net = None
        node_attr = None
        was = None
        ...
        with open(ego_net_path, 'r') as ego_net_f:
            ...
            if len(cur_ego_net[0]) > 0 and cur_ego_id != -1 and (limit is None or n < limit):
                yield cur_ego_id, (node_attr[:was.sum()], np.array(cur_ego_net[0]).T, np.array(cur_ego_net[1]))
...
class LabelDataset(IterableDataset):
    ...
    @staticmethod
    def read_label(label_path, limit=None):
        n = 0
        with open(label_path, 'r') as label_f:
            ...
            if cur_ego_id != -1 and len(cur_label) > 0 and limit is not None and n < limit:
                yield cur_ego_id, np.array(cur_label)
...
class EgoLabelDataset(IterableDataset):
    def __init__(self, ego_net_path, label_path):
        self.ego_net_path = ego_net_path
        self.label_path = label_path

    def __iter__(self):
        ego_iter = EgoDataset(self.ego_net_path)
        label_iter = LabelDataset(self.label_path)
